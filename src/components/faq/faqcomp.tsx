...
//                 <motion.div
//                   key={index}
//                   className="border rounded-2xl shadow-md overflow-hidden hover:bg-gray-50"
//                   variants={{
...
export default function FAQSection() {
  ...
  return (
    <section className="py-16 px-4 xl:px-0 max-w-7xl mx-auto">
      {/* Heading with Animation */}
      <motion.h2
        className="text-5xl font-bold text-black text-center font-playfair-black mt-8"
        initial={{ opacity: 0, y: -20 }}
        whileInView={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.7, ease: "easeOut" }}
        viewport={{ once: true }}
      ...
      <motion.p
        className="text-gray-700 mt-4 text-center font-helvetica-regular text-xl"
        initial={{ opacity: 0, y: 10 }}
        whileInView={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.7, delay: 0.2, ease: "easeOut" }}
        viewport={{ once: true }}
            ...
            {filteredFAQs.length > 0 ? (
              filteredFAQs.map((faq, index) => (
                ...
                >
                  ...
                  <div
                    className="flex justify-between items-center p-10 cursor-pointer" // Increased padding to p-6 for more height
                    onClick={() => toggleFAQ(index)}
                  >
                    <span
                      className={` text-2xl helvetica-neue-bold ${
